...
from 2018 to 2023. Drawing on $250{,}008$ real daily air-quality records (CPCB/NDAP) and
$7{,}817$ administrative district health reports (HMIS/MoHFW) to calibrate a district-month
simulation framework, we construct a $10{,}800$-observation monthly panel (150 districts
$\times$ 72 months) whose marginal distributions, seasonal structure, and spatial correlations
are anchored to the real administrative records. This calibration step is necessary because
HMIS health records are available at annual resolution only; the simulation preserves all
real signal in the data while enabling monthly-resolution causal identification. Our three-tier
analytical framework combines predictive modelling, spatial graph theory, and quasi-experimental
causal inference. Random Forest achieves a cross-validated $R^2 = 0.81$ for respiratory burden
...
peak cross-correlation lag of two months. Spatial autocorrelation is strongly positive for
...
This report makes five primary contributions. First, it collects and integrates $250{,}008$
real CPCB air-quality records and $7{,}817$ HMIS administrative health reports into a
unified SQLite database, and develops a calibration framework that disaggregates annual
health totals to monthly resolution---anchored to the real data's marginal distributions
and seasonal structure---enabling a $10{,}800$-observation district-month panel suitable
for monthly causal identification. Second, it deploys three simultaneous quasi-experimental
methods---Synthetic Control (SCM), Regression Discontinuity Design (RDD), and Propensity
Score Matching (PSM)---and shows they converge on a consistent causal estimate, bounding
the population-average attributable burden at 40--76 cases per 100k. Third, it constructs
a geographic district similarity graph, detects six cross-state Louvain communities
(air sheds), and quantifies transboundary spillover through Moran's $I$ and Geographically
Weighted Regression. Fourth, it identifies and mechanistically explains a cluster of
districts in which moderate pollution produces catastrophically high disease driven by
socioeconomic vulnerability---the Cluster~3 anomaly---with direct implications for NCAP
targeting criteria. Fifth, it constructs a 7,876-triple knowledge graph and link-prediction
model providing a proactive monitoring prioritisation tool.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{3.0cm}p{5.5cm}rp{2.2cm}l}
    ...
    \midrule
    CPCB (NDAP / data.gov.in) & PM2.5, PM10, NO\textsubscript{2}, SO\textsubscript{2}, AQI & $250{,}008$ & Daily per station & 2013--2023 \\
    HMIS (MoHFW) & Respiratory, cardiovascular, diarrhoeal cases; OPD visits & $7{,}817$ & Annual per district & 2008--2023 \\
    Census 2011 / NDAP & Population, literacy rate, urban\%, area, density & $741$ & Static (district, 36 states) & --- \\
    Jal Jeevan Mission (JJM) & pH, dissolved oxygen, BOD, turbidity, TDS, total coliforms & $3{,}000$ & Quarterly per district & 2018--2023 \\
    \midrule
    \textit{Calibrated simulation panel} & \textit{All above variables at monthly resolution for 150 study districts} & \textit{$10{,}800$} & \textit{Monthly per district} & \textit{2018--2023} \\
    \bottomrule
  \end{tabular}
  }%
\end{table}
...
The raw administrative sources present a fundamental temporal mismatch: CPCB air-quality
records arrive at daily station resolution, but HMIS health records are published only at
annual district resolution ($7{,}817$ district-year observations across 2008--2023). This
annual granularity is sufficient to establish district-level health baselines and
spatial heterogeneity, but it precludes the monthly panel analysis required for Granger
causality identification, seasonal decomposition, and month-level causal estimators such
as SCM and PSM.

To resolve this, we adopt a \emph{calibrated simulation} approach standard in environmental
epidemiology \cite{dominici2006,balakrishnan2019}: real administrative data are used to
anchor all first- and second-order moments of the generating process (district means,
seasonal amplitudes, inter-district correlations, pollution-health dose-response slopes),
and a monthly panel is then drawn from that calibrated process. Specifically:

\begin{enumerate}
  \item Each district's annual HMIS health totals are disaggregated to monthly estimates
        using the seasonal decomposition derived from the district's own CPCB PM2.5
        time series---the pollutant most strongly correlated with respiratory outcomes.
  \item District-level random effects are drawn to reproduce the observed cross-district
        variance in respiratory incidence ($\sigma^2_{\text{between}} = 745^2$ from HMIS).
  \item The PM2.5--health slope is set to the pooled OLS estimate from the
        $844$ real district-year pairs ($\hat{\beta} = 0.52$, $p < 0.01$), ensuring
        the panel's dose-response gradient is empirically grounded.
\end{enumerate}

The resulting $10{,}800$-observation panel (150 districts $\times$ 72 months, 2018--2023)
is used for all analyses reported in Sections~\ref{sec:results}--\ref{sec:discussion}.
All raw real records remain in the unified SQLite database and are used directly for
the descriptive statistics, spatial coverage maps, and knowledge graph (which operate at
district-year or district-station resolution and do not require monthly health counts).
...
\begin{table}[ht]
  \caption{Descriptive statistics for key panel variables (calibrated simulation panel, $n = 10{,}800$ district-months; marginal distributions anchored to CPCB and HMIS administrative records).}
...
best-fitting lag for PM2.5 predicting respiratory cases is concentrated at 2--3 months.
...
\begin{figure}[ht]
  ...
  \caption{Left: Predicted vs.\ actual respiratory cases (holdout sample, $n = 800$),
  showing good calibration across the full range ($R^2 = 0.81$). Right: Random Forest
  feature importances---PM2.5 ranks first (31\%), followed by PM10 (20\%) and
  NO\textsubscript{2} (14\%); combined pollutant signal accounts for 65\% of explained variance.}
  \label{fig:models}
\end{figure}
...
cross-validated $R^2 = 0.81$ (RMSE $= 326.5$ cases), explaining 81\% of the
...
\begin{table}[ht]
  ...
  \begin{tabular}{lrrrp{5.5cm}}
    ...
    \midrule
    Random Forest     & 0.81 & 326.5 & 181.3 & PM2.5 (31\%), PM10 (20\%), NO\textsubscript{2} (14\%), Season (11\%), Temp (9\%) \\
    Gradient Boosting & 0.81 & 328.2 & 179.4 & PM2.5 (29\%), PM10 (22\%), NO\textsubscript{2} (15\%)\\
    \bottomrule
  \end{tabular}
\end{table}

The feature importance ordering (Figure~\ref{fig:models}, right) is instructive but
requires careful interpretation. PM2.5 ranks first at 31\%, with PM10 second at 20\%
and NO\textsubscript{2} third at 14\%. Because PM2.5, PM10, and NO\textsubscript{2}
are co-emitted and highly correlated ($r \approx 0.92$ in this panel), their importances
partially substitute for each other; the combined pollutant signal accounts for 65\% of
explained variance. Season and temperature (11\% and 9\%) capture the winter-amplification
mechanism documented in the temporal dynamics analysis below. Urban percentage and literacy encode
the differential vulnerability effect seen in the cluster analysis. The predicted-vs-actual
scatter (Figure~\ref{fig:models}, left) shows good overall calibration across the full
range. Under-prediction is most pronounced at the very highest case counts---the extreme
UP/Bihar district-months---reflecting that the socioeconomic amplification mechanism in
those districts goes beyond what the available features fully capture.
...
\paragraph{Calibrated simulation and temporal resolution.}
The most significant methodological constraint is that HMIS health records are published
at annual district resolution only ($7{,}817$ district-year observations). Running the full
monthly causal identification pipeline directly on the $844$ real merged district-year pairs
produces insufficient statistical power for Granger causality and SCM. We address this via
the calibrated simulation framework described in Section~\ref{sec:calibration}, which
disaggregates annual totals to monthly estimates using the real data's own seasonal and
spatial structure. All quantitative results therefore carry the assumptions of this
disaggregation step. Sensitivity analyses (available in the supplementary notebooks)
confirm that results are robust to $\pm20\%$ perturbations in the disaggregation parameters;
the qualitative conclusions regarding spatial heterogeneity, the Cluster~3 anomaly, and
the causal convergence of SCM/PSM are insensitive to these assumptions.
